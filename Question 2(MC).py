...
ax1.plot(np.log10(timesteps), np.log10(MSE_step))
ax1.set_title('MC error')
plt.xlabel('number of simulation paths')
plt.ylabel('Root Mean-Square Error')
ax2 = plt.subplot2grid(shape = (1,2), loc = (0,1))
ax2.plot(np.log10(num_realisations), np.log10(MSE_path))
...
plt.scatter(np.log10(num_realisations[:-2]), np.log10(MSE_path[:-2]))
slope, intercept = np.polyfit(np.log10(num_realisations[:-2]), np.log10(MSE_path[:-2]),1)
print(slope)
trendline_x = np.array([np.log10(num_realisations[:-2]).min(), np.log10(num_realisations[:-2]).max()])
...
plt.clf()
plt.plot(tt,St[:,0,0], label='0,0')
plt.plot(tt, St[:, 0, 1], label='0,1')
plt.plot(tt, St[:, 1, 0], label='1,0')
plt.plot(tt, St[:, 1, 1], label='1,1')
plt.title('solve the Ricatti ODE ')
...
LQR_1 = LQR_Solver(H, M, sigma, T, C, D, R)


batch_size = 100
tt = np.linspace(0,T,batch_size+1)
St_1 = LQR_1.solve_ricatti(tt).numpy()
...
plt.clf()
plt.plot(tt,St_1[:,0,0], label='0,0')
plt.plot(tt, St_1[:, 0, 1], label='0,1')
plt.plot(tt, St_1[:, 1, 0], label='1,0')
plt.plot(tt, St_1[:, 1, 1], label='1,1')
plt.title('solve the Ricatti ODE with random input')
plt.xlabel('time grid')
plt.ylabel('S(t)')
plt.savefig('St with random input.png')
plt.legend()
