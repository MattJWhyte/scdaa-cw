...
def get_gradient(output, x):
    grad = torch.autograd.grad(output, x, grad_outputs=torch.ones_like(output), create_graph=True, retain_graph=True,
                               only_inputs=True)[0]
    return grad
...
class PDE_DGM_BlackScholes(nn.Module):
    ...
    def fit(self, max_updates: int, batch_size: int, option, device):
        ...
        for it in range(max_updates):
            ...
            pde = grad_u_t + torch.sum(self.mu * input_domain.detach() * grad_u_x, 1,
                                       keepdim=True) + 0.5 * self.sigma ** 2 * laplacian - self.mu * u_of_tx
            ...
            if it % 10 == 0:
                pbar.update(10)
                pbar.write(
                    "Iteration: {}/{}\t MSE functional: {:.4f}\t MSE terminal: {:.4f}\t Total Loss: {:.4f}".format(it,
                                                                                                                   max_updates,
                                                                                                                   MSE_functional.item(),
                                                                                                                   MSE_terminal.item(),
                                                                                                                   loss.item()))
    ...
    def sdeint(self, ts, x0, antithetic=False):
        ...
        for idx, t in enumerate(ts[1:]):
            h = ts[idx + 1] - ts[idx]
            if antithetic:
                brownian_increments[:batch_size // 2, idx, :] = torch.randn(batch_size // 2, self.d,
                                                                            device=device) * torch.sqrt(h)
